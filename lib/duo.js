...
function Duo(root) {
  ...
  this.pluginsPackage = new Step;
  this.pluginsPackage.run = thunk(this.pluginsPackage.run);
}
...
Duo.prototype.usePackage = function (fn) {
  if (Array.isArray(fn)) {
    fn.forEach(this.usePackage, this);
    return this;
  } else {
    if (~this.pluginsPackage.fns.indexOf(fn)) return this;
    var name = fn.name || '(anonymous)';
    this.emit('pluginPackage', name);
    debug('using package plugin: %s', name);
    this.pluginsPackage.use(fn);
    return this;
  }
};
...
Duo.prototype.dependency = function *(dep, file, entry) {
  ...
  // call package plugins
  yield this.pluginsPackage.run(pkg);
